...
def calculate_user_similarity(trainset, use_iif_similarity=False):
    ...
    # count co-rated items between users 统计用户之间相同物品的个数
    print('generate user co-rated movies similarity matrix...')
    # the keys of user_sim_mat are user1's id,
    # the values of user_sim_mat are dicts which save {user2's id: co-occurrence times}.
    # so you can seem user_sim_mat as a two-dim table.
    ...
    for movie, users in movie2users.items():
        for user1 in users:
            ...
            user_sim_mat.setdefault( user1, defaultdict( int ) )
            for user2 in users:
                if user1 == user2:
                    continue
                ...
                if use_iif_similarity:
                    # if the item is very popular, users' similarity will be lower.
                    user_sim_mat[user1][user2] += 1 / math.log( 1 + len( users ) )
                else:
    ...
    # calculate user-user similarity matrix jaccard方法变体
    ...
    for user1, related_users in user_sim_mat.items():
        ...
        for user2, count in related_users.items():
            ...
            user_sim_mat[user1][user2] = count / math.sqrt( len_user1 * len_user2 )
            # log steps and times.
        usersim_mat_time.count_time()

    print('calculate user-user similarity matrix success.')
    usersim_mat_time.finish()
    return user_sim_mat, movie_popular, movie_count
