...
class UserBasedCF:
    ...
    def __init__(self, k_sim_user=20, n_rec_movie=10, use_iif_similarity=False, save_model=True,movie_titles=None):
        """
        ...
        self.movie_titles = movie_titles if movie_titles else {}  # 默认空字典，避免None错误
    ...
    def recommend(self, user):
        ...
        @return: a list of (movie_id, predicted_score)
        """
        ...
        K = self.k_sim_user
        N = self.n_rec_movie
        predict_score = collections.defaultdict(float)
        ...
        for similar_user, similarity_factor in sorted(self.user_sim_mat[user].items(),
            ...
            for movie, rating in self.trainset[similar_user].items():
                if movie in watched_movies:
                    continue
            # 用相似用户的评分×相似度来预测该用户的兴趣度
                predict_score[movie] += similarity_factor * rating

    # 排序后返回前 N 个 (movie, score) 元组
        return sorted(predict_score.items(), key=itemgetter(1), reverse=True)[:N]
    ...
    #     # return the N best score movies
    #     return [movie for movie, _ in
    #             sorted(predict_score.items(), key=itemgetter(1), reverse=True)[0:N]]
    ...
    #         if user not in self.trainset:
    #             print(f"Warning: user {user} not in trainset, skipping...")
    #             continue
    #         rec_movie_ids = self.recommend(user)
    #         if not rec_movie_ids:
    #             print(f"Warning: No recommendation generated for user {user}.")
    #             continue
    #         rec_movie_names = [self.movie_titles.get(int(mid), f"Movie {mid}") for mid in rec_movie_ids]
    #         movies_recommend[user] = rec_movie_names
    ...
    def predict(self, testset):
        ...
        :param testset: test dataset
        :return: dict : {user: [movie title + score]}
        """
        movies_recommend = defaultdict(list)
        print('Predict scores start...')
        ...
        for i, user in enumerate(testset):
            rec_movies = self.recommend(user)  # list of (movie_id, score)
            movies_recommend[user] = [
                f"{self.movie_titles.get(int(mid), f'Movie {mid}')} - 兴趣度: {score:.2f}"
                for mid, score in rec_movies
            ]
            predict_time.count_time()
