...
class MostPopular:
    ...
    def __init__(self, n_rec_movie=10, save_model=True, movie_titles=None):
        """
    ...
    def recommend(self, user):
        ...
        for movie, score in self.movie_popular_sort:
            if len(predict_movies) < N and movie not in watched_movies:
                predict_movies.append((movie, score))
    ...
    def predict(self, testset):
        """
        Recommend movies (name + popularity score) to all users in testset.
        :param testset: test dataset
        :return: dict: {user: ["电影名：热度值", ...]}
        """
    ...
    #     for i, user in enumerate(testset):
    #         rec_movies = self.recommend(user)  # type:list
    #         movies_recommend[user] = [self.movie_titles.get(int(mid), f"Movie {mid}") for mid in rec_movies]
