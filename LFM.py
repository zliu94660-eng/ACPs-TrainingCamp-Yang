...
# -*- coding: utf-8 -*-
"""
Recommend according to the user's favorite categories
...
class LFM:
    ...
    def __init__(self, K, epochs, alpha, lamb, n_rec_movie=10, save_model=True, movie_titles=None):
        """
        ...
        self.movie_titles = movie_titles or {}  # 新增行
        ...
        for epoch in range(self.epochs):
            ...
            for user in trainset:
                ...
                for item, rui in samples.items():
                    eui = rui - self.prediction(user, item)
                    for k in range(self.K):
    ...
    def predict(self, testset):
        """
        Recommend movie names + scores to all users in testset.
        :param testset: test dataset
        :return: dict: {user: ["电影名: 兴趣度", ...]}
        """
        ...
        for i, user in enumerate(testset):
            ...
            rec_named = [
                f"{self.movie_titles.get(int(mid), f'Movie {mid}')}：{score:.4f}"
                for mid, score in rec_movies
            ]
            movies_recommend[user] = rec_named
    ...
    # def predict(self, testset):
    #     """
    #     Recommend movie names to all users in testset.
    #     :param testset: test dataset
    #     :return: dict: {user: [recommended movie names]}
    #     """
    #     movies_recommend = defaultdict(list)
    #     print('Predict scores start...')
    #     predict_time = utils.LogTime(print_step=500)
    #     for i, user in enumerate(testset):
    #         rec_movies = self.recommend(user)  # already names
    #         rec_names = [self.movie_titles.get(int(mid), f"Movie {mid}") for mid in rec_movies]
    #         movies_recommend[user] = rec_names
    #         predict_time.count_time()
    #     print('Predict scores success.')
    #     predict_time.finish()
    #     return movies_recommend
