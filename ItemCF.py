...
class ItemBasedCF:
    ...
    def __init__(self, k_sim_movie=20, n_rec_movie=10, use_iuf_similarity=False, save_model=True, movie_titles=None):
        """
    ...
    def predict(self, testset):
        ...
        for i, user in enumerate(testset):
            ...
            rec_movies = self.recommend(user)  # [(movie_id, score), ...]
            if not rec_movies:
                print(f"Warning: No recommendations found for user {user}.")
                continue
            ...
            movies_recommend[user] = rec_movie_info
    ...
    #     predict_time = LogTime(print_step=500)
    #     for i, user in enumerate(testset):
    #         rec_movie_ids = self.recommend(user)  # type:list
    #         rec_movie_names = [self.movie_titles.get(mid, mid) for mid in rec_movie_ids]
    #         movies_recommend[user] = rec_movie_names
    #         # log steps and times.
    #         predict_time.count_time()
    #     print('Predict scores success.')
